...
if(!empty($_FILES["sscat_cover"]["name"])) 
{
	...
	$LogoData = mysqli_query($servercnx,"SELECT sscat_cover FROM sscat WHERE id = '$id'");
	 $LogoImage = mysqli_fetch_array($LogoData);
	 $sscat_cover = $LogoImage['sscat_cover'];
	 if ($sscat_cover != 'default.jpg') { unlink($path.$sscat_cover);	unlink($path .'thumb/'. $sscat_cover); }
	// $sscat_cover='Cover-iii'.$sscat_position.'-'.time().'.jpg';
...
else{ $LogoData = mysqli_query($servercnx,"SELECT sscat_cover FROM sscat WHERE id = '$id'"); $LogoImage = mysqli_fetch_array($LogoData); $sscat_cover = $LogoImage['sscat_cover']; }
...
&& isset($sscat_uniid)
 ...
 $result = mysqli_query($servercnx,"UPDATE sscat SET

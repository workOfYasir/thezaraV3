...
// Page CoverLogoData = mysqli_query($servercnx,"SELECT slide_image FROM slidei WHERE id = '$id'");slide_image
if(!empty($_FILES["slide_image"]["name"])) 
	{
		...
		$slide_image='Cover-'.$blog_date.'-'.time().'.'.end($temp);
		$image_info = getimagesize($newname);
		$image_width = $image_info[0];
		$image_height = $image_info[1];
	} 
	else{ $LogoData = mysqli_query($servercnx,"SELECT slide_image FROM slidei WHERE id = '$id'"); $LogoImage = mysqli_fetch_array($LogoData); $slide_image = $LogoImage['slide_image']; 	

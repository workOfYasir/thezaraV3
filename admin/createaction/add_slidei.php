...
if(!empty($_FILES["slide_image"]["name"])) 
{
	...
	$path= '../../images/Home/';
	$newname=$path.'Home-'.time().'.'.end($temp);
	move_uploaded_file($_FILES['slide_image']['tmp_name'],$newname);
	$slide_image='Home-'.time().'.'.end($temp);
	$image_info = getimagesize($newname);
	$image_width = $image_info[0];
	$image_height = $image_info[1];
} 
else{ $slide_image='default.jpg'; 	$path= '../../images/Home/';
...
if (
	...
	if ($result == true) {
		...
		echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=../View-Slidei.php">';
	}
} else {
